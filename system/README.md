...
- **No `theme/` dependency.** Utilities are inlined, like `ui/`. `icons/` is used for the three
  glyphs these components draw (`IconChevronLeft`, `IconChevronRight`, `IconSun`, `IconMoon`,
  `IconThemeAuto`) rather than duplicating SVG.
...
- **No `Shell`, `Nav`, `Auth`, `Menu`, `Header`, `ProfileDropdown`, `ImageGallery`, `StateInit`,
  `LeadForm` or `NewsletterForm`.** See below.
...
| `ImageGallery`                            | `<dialog>` lightbox plus keyboard and snap-scroll behaviour; needs a DOM test harness this repo does not have yet.                                                                                           |
| `StateInit`, `LeadForm`, `NewsletterForm` | The first is an app's hydration bridge (env keys hardcoded), the other two are app-specific form chrome with anti-bot fields the host must render.                                                           |
...
starts on any weekday, the midnight-wrapping period buckets, the theme cycle, and route ordering.
`SWUpdater`'s listeners are driven by a fake registration, so the update path is exercised without
a browser.
