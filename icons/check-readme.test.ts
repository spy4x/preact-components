/**
 * Guards the ported set's bookkeeping in `README.md`.
 *
...
import { audit, BARREL_EXPORTED, PORTED_FILES } from "./check-readme.ts"

const { findings, checked } = await audit()

describe("ported-set bookkeeping in README.md", () => {
  it("accounts for every file in the pinned ported-set inventory", () => {
    ...
    expect(PORTED_FILES.length).toBe(52)
    expect(new Set(PORTED_FILES).size).toBe(PORTED_FILES.length)
    expect(BARREL_EXPORTED.length).toBe(49)
  })

  it("agrees with the module, naming every mismatch", () => {
