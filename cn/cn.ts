 ...
 *
 * Falsy inputs are dropped by `clsx`. The surviving string is then passed
 * through `twMerge`, so a later utility wins over an earlier one in the same
 * group (`cn("p-2", "p-4")` is `"p-4"`) instead of both reaching the DOM and
 * leaving the winner to CSS source order.
 *
