...
export type Status = "ready" | "beta" | "wip" | "paused" | "archived" | "known-issue"
...
/** Tone per status, read through the shared colour tokens — never a literal palette value. */
 ...
 */
const SHAPES: Record<Status, (props: { class?: string }) => JSX.Element> = {
  ready: ReadyShape,
  beta: BetaShape,
  wip: WipShape,
  paused: PausedShape,
 ...
 *
 * A new component beside `Badge`, not a change to it: `Badge` is colour-plus-text with no shape,
 * and giving it one would mean every existing `Badge` caller inheriting an icon it never asked for.
 */
export function StatusMark({ status, label, class: className }: StatusMarkProps): JSX.Element {
  const Shape = SHAPES[status]
  return (
    <span
      class={cn("inline-flex items-center gap-1.5 text-sm font-medium", TONES[status], className)}
    >
