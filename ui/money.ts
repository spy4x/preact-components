 ...
 * turns typed text back into one. `money-display.tsx` and `money-input.tsx` are this module's only
 * callers.
 *
 * Package-private on purpose: not in `ui/deno.json`'s `exports`, not re-exported from
 * `ui/+index.ts`. These functions belong in `spy4x/ts-libs`, because servers need them too — see
 * `spy4x/ts-libs#189`. That PR is open but not released, so this is a byte-identical-in-behaviour
 * copy kept here until a release lands; `#275` tracks importing it from `@spy4x/platform` and
 * deleting this file. `Result`/`ok`/`err` below are copied from `ts-libs`'s
 * `platform/universal/result.ts` for the same reason, rather than pulled in as a dependency on an
 * unreleased package.
 *
 ...
 * step for a rounding error to hide in. `parseFloat("1.005") * 100` is `100.49999999999999` — a
 * value with more fraction digits than the currency allows is refused, never rounded, because
 * rounding a typed amount can move a cent that was never approved. {@link formatMoney} divides the
 * integer amount by a power of ten, which is safe for every integer this module accepts: the
 * nearest `double` to `amount / 10 ** decimals` is always within far less than half a unit of the
 * last decimal place for any `amount` inside `Number.MAX_SAFE_INTEGER`, so formatting it back to
 * exactly `decimals` fraction digits always reproduces the exact decimal value.
 */

/** A value or an error, as a discriminated union on `ok`. Copied from `ts-libs`, see module doc. */
